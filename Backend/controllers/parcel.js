...
export const fetchAllParcels = async (req, res) => {
  try {
    ...
    if(!parcels)
      return res.status(404).json("no parcels found");
...
export const updateParcel = async (req, res) => {
  ...
  if (!parcel)
    return res.status(404).json('No package found');
...
export const getUserParcel = async (req, res) => {
  ...
  try {
    const userParcels = await ParcelModel.find({ senderemail: email }).sort('-createdAt');

    if(!userParcels)
      return res.status(404).json("No parcel found for user");
...
export const deleteParcel = async (req, res) => {
  ...
  try{
    ...
    if(!deletedParcel)
      return res.status(404).json("No such parcel found");
...
export const userParcels = async (req, res) => {
  ...
  try {
    ...
    if(!userParcels)
      return res.status(404).json("No parcel found");
